window.C1DropDown = {
    init: function (header, componentReference) {
...
window.C1TextBox = {
    ...
    onChange: async function (e) {
        ...
        await window.C1TextBox.notifyFinishedTextChange(input, currentValue);
    },
    onInput: async function (e) {
        ...
        var input = e.currentTarget;
        var currentValue = input.value;
        window.C1TextBox.setText(input, currentValue);
        await window.C1TextBox.notifyTextChange(input, currentValue);
    },
    notifyFinishedTextChange: async function (input, currentValue) {
    ...
    onKeyDown: async function (e) {
        ...
        await invokeOnTextBoxKeyDownAsync();
        ...
        async function invokeOnTextBoxKeyDownAsync() {
            var componentReference = input.componentReference;
            var semaphore = input.Semaphore;
            try {
                await semaphore.waitAsync();
...
window.C1MaskedTextBox = {
    init: function (input, autoComplete, delay, text, componentReference, maskSettings) {
        ...
        input.onfocus = function (e) {
            invokeComponentReferenceMethod('OnFocusInAsync');
        }

        input.onblur = function (e) {
            invokeComponentReferenceMethod('OnFocusOutAsync');
        }
...
window.C1NumericEditor =
{
    ...
    onchange: function (e, settings, sign) {
        ...
        if (e.currentTarget.value !== undefined && e.currentTarget.value !== null && !sign) {
            if (e.currentTarget.value[0] === '+') { sign = '+' }
            if (e.currentTarget.value[0] === '-') { sign = '-' }
        }
        ...
        if (currentTarget.value !== 0 && !currentTarget.value) { // case user input is blank | undefined | null
        ...
        var numericValue = extractValue(currentTarget, settings);
        ...
        function extractValue(currentTarget, settings) {
            ...
            if (settings.testValue) {
                ...
                if (C1NumericEditor.utils.getDecimalSeparatorKeyCode(settings.testValue) === 46) {
                    numericValue = Number(
                        C1NumericEditor.utils
                            .cleanCharByRegex(currentTarget.value, /\,/g)
                ...
                numericValue = Number(currentTarget.value);
            }
        ...
        function changeValue(value, sign) {
            ...
            if (hasMax && value > settings.max) {
                ...
                componentReference.invokeMethodAsync('OnChangedAsync', String(settings.max), null);
                return;
            }
            if (hasMin && value < settings.min) {
                currentTarget.value = settings.min;
                componentReference.invokeMethodAsync('OnChangedAsync', String(settings.min), null);
                return;
            }

            componentReference.invokeMethodAsync('OnChangedAsync', String(value), sign);
        }
    ...
    onfocus: function (e) {
        var currentTarget = e.currentTarget;
        var componentReference = currentTarget.componentReference;
        componentReference.invokeMethodAsync('OnFocusAsync');

    },
    onblur: function (e) {
        var currentTarget = e.currentTarget;
        var componentReference = currentTarget.componentReference;
        componentReference.invokeMethodAsync('OnFocusOutAsync');
    },
    utils: {
        ...
        getDecimalSeparatorKeyCode: function (testValue) {
            ...
            // ##,###.##
            if (lastDotIndex > lastCommandIndex) return 46;

            // ##.###,##
            if (lastCommandIndex > -1 && lastCommandIndex > lastDotIndex) return 44;
...
/************ C1Popup ************/
window.C1Popup = {
    init: function (overlay, popup, ownerId, openTrigger, closeTrigger, openDelay, closeDelay, gap,
        position, button, isFullScreen, isDraggable, isResizable, isModeless, dimensions, componentReference) {
        ...
        if (ownerId) {
            ...
            overlay.popup.owner = ownerId ? document.getElementById(ownerId) : null;
        ...
        function listenGlobalEvents() {
            ...
            var adjustPositionCallback = function () {
            ...
            document.addEventListener('scroll', adjustPositionCallback, true);
            ...
            addEventListener('keydown', function (e) {
                ...
                else if (e.target === overlay.header) { e.preventDefault(); }
...
window.Core = {
    ...
    getPopupPosition: async function (popup, bounds, position, ptOffset, gap = 0) {
        // get reference rect
        var SDMINWIDTH = 200, doc = document.documentElement, scrWid = doc.clientWidth, //innerWidth,
        ...
        pos.y = pos.y + ptOffset.y;
        // done
        return pos;

        async function fitHeight(space) {
            if (space < sz.height) {
                var popupScrollContent = popup.getElementsByClassName("c1ScrollViewer");

                if (space > 80 && popupScrollContent && popupScrollContent.length > 0 && !popupScrollContent[0].style.maxHeight) {
                    popupScrollContent[0].style.maxHeight = space + "px";
                }
            }
            sz = new window.Core.Utils.Size(popup.offsetWidth + (mx || 0), popup.offsetHeight + (my || 0));
        }
